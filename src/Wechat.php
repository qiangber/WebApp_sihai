  ...
  class Wechat {
    ...
    public function run() {
      switch ($this->getRequest('msgtype')) {
        ...
        case 'text':
          if ($this->getRequest('Content')=='happy new year') {
            $items = array(
              new NewsResponseItem('测试', '祝大家新年快乐', 'http://1.wxphptest.sinaapp.com/happyNewYear.jpg', 'http://www.rabbitpre.com/m/URuUZvaqE')
            );
            exit(new NewsResponse($this->getRequest('fromusername'), $this->getRequest('tousername'), $items, 0));
          } else {
            $this->onText();
          }      
          break;
